...
5. Build a green-preferred luma proxy without demosaicing every input frame.
...
8. Merge using an ISO-interpolated read/shot-noise variance and two robust Huber passes. Saturated samples are excluded when an unsaturated observation exists.
...
11. Apply the configurable camera matrix, mild chroma-only spatial suppression, scene-referred tone, local contrast and halo-limited detail.
